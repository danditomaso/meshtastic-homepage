...
// Initial messages shown on load, max messages kept in history for scrolling
const INITIAL_MESSAGE_COUNT = 5;
const MAX_VISIBLE_MESSAGES = 15;

// First message comes quick so the demo feels alive immediately
...
export function useDeviceMockup({
	...
	const pushMessage = useCallback((msg: BuiltMessage) => {
		// Track this message as "new" for animation
		...
		// Prepend new message (newest first for flex-col-reverse display)
	...
	// Stable reference to pushMessage for the generator effect
	const onTimelineMessage = useEffectEvent((msg: BuiltMessage) => {
		pushMessage(msg);
	});

	// Timeline playback via async generator
	useEffect(() => {
		if (timeline.length === 0) return;
