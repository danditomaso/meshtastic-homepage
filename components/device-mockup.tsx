...
import { hydrateUsers, buildTimeline } from "@/lib/build-timeline";
import type { BuiltMessage } from "@/types/conversation.types";
import emojisData from "@/data/emojis.json" with { type: "json" };
import usersData from "@/data/users.json" with { type: "json" };
import conversationsData from "@/data/conversations.json" with { type: "json" };
import keyboardData from "@/data/keyboard-layout.json" with { type: "json" };
					...
					<div className="flex items-center gap-1">
						...
						<Popover open={pickerOpen} onOpenChange={setPickerOpen}>
							<PopoverTrigger asChild>
								<button
									type="button"
									className="rounded-md p-1 text-primary/60 hover:bg-primary/20 hover:text-primary will-change-[background-color]"
									title="React"
								>
						...
						<div className="flex shrink-0 items-center gap-1">
							...
							<Popover open={pickerOpen} onOpenChange={setPickerOpen}>
								<PopoverTrigger asChild>
									<button
										type="button"
										className="rounded-md p-1 text-primary/60 hover:bg-primary/20 hover:text-primary"
										title="React"
									>
										<Smile className="h-5 w-5" />
									</button>
						...
						<div className="bg-gray-900/50 px-4 pb-3 pt-4">
							<div className="flex items-center gap-2 rounded-full bg-gray-800 px-4 py-2.5">
								<input
									...
									className="flex-1 bg-transparent font-mono text-base text-gray-200 placeholder-gray-500 outline-none caret-primary"
								/>
								<button
									type="button"
											...
											{key === "😊" ? (
												<Popover>
													<PopoverTrigger asChild>
														<button
															type="button"
															className="flex h-7 w-10 items-center justify-center rounded-md bg-gray-800/80 text-base font-medium text-gray-200 transition-colors hover:bg-primary hover:text-gray-900 active:scale-95"
														>
												...
												<button
													...
													className={`flex items-center justify-center rounded-md bg-gray-800/80 font-mono text-xs font-medium text-gray-200 transition-colors hover:bg-primary hover:text-gray-900 active:scale-95 ${
														key === "space"
															? "h-7 flex-1"
															: key === "↵"
